...
func validateCases(root string, raw []rawCase) ([]Case, error) {
	...
	for index, item := range raw {
		...
		cases = append(cases, Case{ID: identifier, Prompt: item.Prompt, ExpectedOutput: item.ExpectedOutput, Files: item.Files, Assertions: item.Assertions, RequiredActions: item.RequiredActions})
	}
	return cases, nil
}
