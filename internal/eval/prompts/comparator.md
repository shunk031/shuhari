...
- `A` and `B` are blinded artifact views; `A_response` and `B_response` are the raw agent responses.
...
For each case, choose the materially more useful output. Use `tie` when neither is better. Give a concrete, non-empty reason.
